...
/**
 * @function HudChip
 * @description Small UI component for displaying proctoring status indicators.
 * @param {string} label - The indicator label.
 * @param {string|number} value - The current value.
 * @param {boolean} good - Whether the status is optimal.
 */
 ...
 * @description The main exam interface with integrated AI proctoring and automated submission.
 */
export default function QuizInterface({ quizId, questions, subject, stream, onExit }) {
   ...
   * @description Finalizes the quiz and pushes data to the backend.
   * @param {boolean} autoTerminated - Whether the submission was forced by proctoring.
   */
  ...
  /**
   * @description Security: Prevent context menu and devtools shortcuts.
   */
  useEffect(() => {
  ...
  /**
   * @description Security: Detect tab switching or window blurring.
   */
  useEffect(() => {
      ...
      if (document.hidden) {
        setTabWarnings(prev => {
          ...
          if (next >= 2) {
            ...
            toast.warning("WARNING: Navigating away from the exam tab is not allowed.");
          }
          return next;
        });
      }
  ...
  /**
   * @description Sync webcam stream to the UI bubble.
   */
  useEffect(() => {
  ...
  /**
   * @description Identity verification state management.
   */
  useEffect(() => {
    if (!isVerified && peopleCount === 1) {
      if (warning?.includes('Identity Mismatch')) {
        ...
        <div className="relative z-10 flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-400">
              Exam Subject: <span className="font-bold text-zinc-200">{subject || 'Skill Assessment'}</span>
            </p>
          </div>
          <div className="flex flex-col items-end">
            <div className="rounded-2xl border border-white/10 bg-black/40 px-4 py-2 text-xl font-black text-white shadow-inner">
              {mm}:{ss}
            </div>
          ...
          <div className="rounded-[2rem] border border-white/10 bg-white/5 p-8 shadow-2xl backdrop-blur-3xl">
            {verificationFailed ? (
              ...
              <div className="text-center p-10">
                <p className="text-red-400 font-bold">Error: Question stream corrupted or empty.</p>
              </div>
            ) : (
              <>
                ...
                {activeQuestion?.type === 'MCQ' ? (
                  ...
                  <textarea
                    ...
                    onChange={(e) => setResponses(p => ({...p, [index]: e.target.value}))}
                    className="w-full h-48 rounded-2xl border border-white/10 bg-black/20 p-6 text-white outline-none focus:border-orange-500 transition-all placeholder:text-zinc-600"
                    placeholder="Provide your subjective response here..."
                  />
                )}
                ...
                <div className="mt-10 flex gap-4">
                  ...
                  {index < questions.length - 1 ? (
                    ...
                    <button onClick={() => handleAutoSubmit()} disabled={isSubmitting} className="px-8 py-3 rounded-xl bg-emerald-500 text-white font-bold shadow-lg shadow-emerald-500/20">Finalize Submission</button>
                  )}
                </div>
              </>
            )}
            ...
            <div className="rounded-[2rem] border border-white/10 bg-white/5 p-6 backdrop-blur-xl">
               ...
               <div className="space-y-4">
                  <div className="flex justify-between items-center">
                     <span className="text-xs text-zinc-400">Eye Contact</span>
                     <span className="text-xs text-emerald-400 font-bold">Stable</span>
                  </div>
                  <div className="flex justify-between items-center">
                     <span className="text-xs text-zinc-400">Identity Lock</span>
                     <span className="text-xs text-emerald-400 font-bold">{isVerified ? 'Active' : 'Pending'}</span>
                  </div>
                  <div className="flex justify-between items-center">
                     <span className="text-xs text-zinc-400">Integrity Score</span>
                     <span className="text-xs text-emerald-400 font-bold">98.5%</span>
                  </div>
               </div>
            </div>
          </div>
