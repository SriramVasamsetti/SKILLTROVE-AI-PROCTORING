...
export default function QuizInterface({ quizId, questions, subject, stream, onExit }) {
  ...
  const handleAutoSubmit = async (autoTerminated = false) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/quiz/${quizId}/submit`, { // ✅ fixed
        method: 'POST',
        headers: {
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-[2rem] border border-white/10 bg-white/5 p-6 backdrop-blur-xl">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center"><span className="text-xs text-zinc-400">Eye Contact</span><span className="text-xs text-emerald-400 font-bold">Stable</span></div>
                ...
                <div className="flex justify-between items-center"><span className="text-xs text-zinc-400">Integrity Score</span><span className="text-xs text-emerald-400 font-bold">98.5%</span></div>
              </div>
            </div>
          </div>
        </div>
